...
$VERIFY="$PROJECT\WINDOWS_EXE_INSTALLER_CLEAN_COMPILE_V5_$TS"

# Important: stage NOT inside BACKUPS, otherwise clean checker sees "backup" in full path.
$STAGE="$PROJECT\WINDOWS_INSTALLER_CLEAN_STAGE_V5_$TS"
...
Write-Host "2) Frontend production build..." -ForegroundColor Cyan
...
bad_dir_names = {
    ...
    ".mypy_cache", ".ruff_cache", ".git", "backups"
}
...
def is_bad_source_path(src: Path) -> bool:
    parts = [p.lower() for p in src.parts]
    if any(p in bad_dir_names for p in parts):
        return True
...
def is_bad_stage_relative(rel: Path) -> bool:
    # IMPORTANT: check only relative path inside clean stage, not absolute S:\...\BACKUPS path.
...
for src in app_dir.rglob("*.py"):
    ...
    if copy_file(src, STAGE / "backend" / "app" / rel):
        copied.append(str(src))

# backend/alembic: only clean migration files
alembic_dir = BACKEND / "alembic"
if not alembic_dir.exists():
    raise SystemExit("backend/alembic not found")

for src in alembic_dir.rglob("*"):
    if src.is_file() and src.suffix.lower() in [".py", ".mako", ".md"]:
        rel = src.relative_to(alembic_dir)
        if copy_file(src, STAGE / "backend" / "alembic" / rel):
            copied.append(str(src))
...
for src in dist_dir.rglob("*"):
    if src.is_file():
        ...
        if copy_file(src, STAGE / "frontend" / "dist" / rel):
            copied.append(str(src))

# deploy/windows only .bat + .md
windows_dir = DEPLOY / "windows"
if not windows_dir.exists():
    raise SystemExit("deploy/windows not found")

for src in windows_dir.rglob("*"):
    if src.is_file() and src.suffix.lower() in [".bat", ".md"]:
        rel = src.relative_to(windows_dir)
        if copy_file(src, STAGE / "deploy" / "windows" / rel):
            copied.append(str(src))
...
for p in STAGE.rglob("*"):
    ...
    if is_bad_stage_relative(rel):
        bad_found.append(str(rel))
...
(VERIFY / "03_clean_stage_report.json").write_text(json.dumps(report, indent=2), encoding="utf-8")

print("STAGE:", STAGE)
print("COPIED_COUNT:", len(copied))
print("MISSING_REQUIRED:", missing)
print("BAD_FOUND_COUNT:", len(bad_found))
print("BAD_FOUND:", bad_found[:30])
...
'@.Replace("__PROJECT__", $PROJECT.Replace("\", "\\")).Replace("__STAGE__", $STAGE.Replace("\", "\\")).Replace("__VERIFY__", $VERIFY.Replace("\", "\\")) |
Set-Content "$VERIFY\03_build_clean_stage_v5.py" -Encoding UTF8

& $PY "$VERIFY\03_build_clean_stage_v5.py" | Tee-Object "$VERIFY\03_build_clean_stage_v5.txt"
if ($LASTEXITCODE -ne 0) { throw "Clean stage v5 build failed" }

Write-Host "4) Create clean Inno Setup script..." -ForegroundColor Cyan

$OUTPUT="$INSTALLER\OutputCleanV5"
$CLEAN_ISS="$INSTALLER\S4_FAMILY_FINANCE_143_CLEAN_V5_$TS.iss"
...
OutputBaseFilename=S4-FAMILY-FINANCE-143-Setup-CLEAN-V5
Compression=lzma
...
Write-Host "5) Compile clean installer v5..." -ForegroundColor Cyan

Set-Location $INSTALLER

& $ISCC $CLEAN_ISS /O"$OUTPUT" /F"S4-FAMILY-FINANCE-143-Setup-CLEAN-V5-$TS" | Tee-Object "$VERIFY\04_inno_clean_v5_compile_output.txt"
if ($LASTEXITCODE -ne 0) { throw "Clean installer v5 compile failed" }

Write-Host "6) Verify compile output clean..." -ForegroundColor Cyan

$compileText = Get-Content "$VERIFY\04_inno_clean_v5_compile_output.txt" -Raw
...
if ($dirtyFound.Count -gt 0) {
  $dirtyFound | Set-Content "$VERIFY\05_dirty_words_found.txt" -Encoding UTF8
  throw "Dirty words found in clean compile output"
}

"dirty_words_found: []" | Set-Content "$VERIFY\05_dirty_words_found.txt" -Encoding UTF8
...
$exeCandidates = Get-ChildItem $OUTPUT -File -Filter "*.exe" |
  Where-Object { $_.Name -like "S4-FAMILY-FINANCE-143-Setup-CLEAN-V5*.exe" } |
  Sort-Object LastWriteTime -Descending

if (-not $exeCandidates -or $exeCandidates.Count -eq 0) {
  throw "Clean V5 setup EXE not found"
}
...
if ($EXEINFO.Length -le 0) {
  throw "Clean V5 EXE size is zero"
}

$FINALCOPY="$BACKUPROOT\S4-FAMILY-FINANCE-143-WINDOWS-SETUP-INSTALLER-CLEAN-V5-$TS.exe"
Copy-Item $EXE $FINALCOPY -Force
$FINALINFO=Get-Item $FINALCOPY

@"
S4 FAMILY FINANCE 143 - WINDOWS CLEAN EXE INSTALLER COMPILE V5 REPORT
...
"@ | Set-Content "$VERIFY\WINDOWS_CLEAN_EXE_INSTALLER_COMPILE_V5_REPORT.txt" -Encoding UTF8

Write-Host "WINDOWS CLEAN EXE INSTALLER COMPILE V5 PASS" -ForegroundColor Green

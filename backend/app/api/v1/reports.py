...
def wallet_report(
    ...
    for wallet in wallets:
        ...
        opening_balance = Decimal(wallet.opening_balance or 0)
        balance = opening_balance + inflow - outflow
...
@router.get("/savings-trend/{family_id}")
def savings_trend_report(
    family_id: str,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Named savings trend surface for Reports charts."""
    require_report_access(db, family_id, current_user.id)
    savings = (
        db.query(SavingsGoal)
        ...
        points.append(
            {
                ...
                "cumulative_saved": money(cumulative),
            }
        )
    return {
        "family_id": family_id,
        "point_count": len(points),
        "total_saved": money(cumulative),
        "trend": points,
        "chart": {
            "bar": [{"label": (p["label"] or "")[:8], "value": float(p["saved"])} for p in points],
            "line": [{"label": str(i + 1), "value": float(p["cumulative_saved"])} for i, p in enumerate(points)],
        },
    }
...
def trial_balance_currency_report(
    ...
    ledger = accounting_service.generate_trial_balance(db, family_id)

    # Keep legacy currency-report shape while sourcing rows from CoA engine
    family = db.get(Family, family_id)
    if not family:
        return {"detail": "Family not found"}
    ...
    for item in ledger.get("rows") or []:
        debit = Decimal(item.get("debit") or 0)
        credit = Decimal(item.get("credit") or 0)
        rate = report_currency_rate(db, item.get("currency") or base_currency, base_currency)
        debit_b = debit * rate
        credit_b = credit * rate
        debit_total += debit_b
        credit_total += credit_b
        rows.append({
            "account_name": item.get("account_name"),
            "account_type": item.get("coa_class") or item.get("account_type"),
            "debit_base": money(debit_b),
            "credit_base": money(credit_b),
        })

    return {
        "family_id": family_id,
        "base_currency": base_currency,
        "engine": "accounting_service.generate_trial_balance",
...
def profit_loss_currency_report(
    ...
    base_currency = family.default_currency
    engine = accounting_service.generate_income_statement(db, family_id)

    # Date-filtered category view kept for UI; engine totals attached for CoA parity
    ...
    for tx in txs:
        ...
        if tx.category_id:
            ...
            if cat:
                cat_name = cat.name_en or cat.name_bn or cat_name
        if ttype == "INCOME":
            income_total += converted
            income_by_category[cat_name] = income_by_category.get(cat_name, Decimal("0")) + converted
        elif ttype == "EXPENSE":
            expense_total += converted
            expense_by_category[cat_name] = expense_by_category.get(cat_name, Decimal("0")) + converted

    net_profit = income_total - expense_total
    income_rows = [{"category": k, "amount_base": money(v)} for k, v in income_by_category.items()]
    expense_rows = [{"category": k, "amount_base": money(v)} for k, v in expense_by_category.items()]

    return {
        "family_id": family_id,
        "base_currency": base_currency,
        "filters": {"start_date": start_date, "end_date": end_date},
        "engine": "accounting_service.generate_income_statement",
        "coa_income_statement": engine,
        "summary": {
...
def executive_dashboard_report(
    ...
    def safe_percent(value: Decimal) -> str:
        try:
            return str(value.quantize(Decimal("0.01")))
        except Exception:
            return "0.00"

    dashboard = report_dashboard(family_id, db, current_user) or {}
    categories = category_wise_report(family_id, None, None, db, current_user) or {}
    goal_analytics = goal_analytics_report(family_id, db, current_user) or {}
    loan_analytics = loan_analytics_report(family_id, db, current_user) or {}

    d = dashboard.get("dashboard", {}) if isinstance(dashboard, dict) else {}
    category_summary = categories.get("summary", {}) if isinstance(categories, dict) else {}
    g = goal_analytics.get("summary", {}) if isinstance(goal_analytics, dict) else {}
    l = loan_analytics.get("summary", {}) if isinstance(loan_analytics, dict) else {}
    dashboard_summary = dashboard.get("summary", {}) if isinstance(dashboard, dict) else {}
    ...
    active_budgets = len(active_budget_rows)

    total_budget_amount = Decimal("0")
    total_budget_spent = Decimal("0")
    over_budget_count = 0

    for budget in active_budget_rows:
        budget_amount = safe_decimal(getattr(budget, "budget_amount", "0"))
        spent_amount = safe_decimal(getattr(budget, "spent_amount", "0"))

        total_budget_amount += budget_amount
        total_budget_spent += spent_amount

        is_over_budget = bool(getattr(budget, "is_over_budget", False))
        if is_over_budget or (budget_amount > 0 and spent_amount > budget_amount):
            over_budget_count += 1

    budget_remaining = total_budget_amount - total_budget_spent
    budget_usage_percent = (
        (total_budget_spent / total_budget_amount) * Decimal("100")
    ...
    direct_wallet_balance = sum(
        (safe_decimal(getattr(wallet, "balance", "0")) for wallet in wallet_rows),
        Decimal("0"),
    )

    wallet_currency_rows = (
    ...
    currencies_detected = sorted(
        {
            str(row[0])
            for row in wallet_currency_rows
            if row and row[0] is not None and str(row[0]).strip()
        }
    )

    mixed_currency = len(currencies_detected) > 1

    total_income = safe_decimal(dashboard_summary.get("total_income", d.get("total_income", "0")))
    total_expense = safe_decimal(dashboard_summary.get("total_expense", d.get("total_expense", "0")))
    cashflow = safe_decimal(dashboard_summary.get("net_income_expense", d.get("cashflow", "0")))
    total_savings = safe_decimal(d.get("total_savings", "0"))
    net_worth = safe_decimal(d.get("net_worth", "0"))
    wallet_report_data = wallet_report(family_id, db, current_user) or {}
    wallet_report_summary = wallet_report_data.get("summary", {}) if isinstance(wallet_report_data, dict) else {}
    wallet_balance = safe_decimal(
        wallet_report_summary.get(
            "total_balance",
            dashboard_summary.get("total_wallet_balance", d.get("wallet_balance", "0")),
        )
    )

    goal_progress_percent = safe_decimal(g.get("overall_progress_percent", "0"))
    goal_target_amount = safe_decimal(g.get("total_target_amount", "0"))
    goal_saved_amount = safe_decimal(
        g.get(
            "total_current_amount",
            g.get("total_saved_amount", g.get("current_amount", "0")),
        )
    )
    if goal_saved_amount == 0 and goal_target_amount > 0 and goal_progress_percent > 0:
        goal_saved_amount = (goal_target_amount * goal_progress_percent) / Decimal("100")

    given_remaining = safe_decimal(l.get("given_remaining", "0"))
    taken_remaining = safe_decimal(l.get("taken_remaining", "0"))

    loan_exposure = given_remaining + taken_remaining
    net_loan_position = given_remaining - taken_remaining
        ...
        alerts.append(
            {
                "type": "NO_WALLET",
                "severity": "HIGH",
                "message": "No active wallet found for this family.",
            }
        )
        ...
        alerts.append(
            {
                ...
                "severity": "HIGH",
                "message": "Cashflow is negative. Expenses/outflows are higher than inflows.",
            }
        )

    if total_expense > 0 and total_savings < (total_expense * Decimal("0.10")):
        alerts.append(
            {
                "type": "LOW_SAVINGS",
                "severity": "MEDIUM",
                "message": "Savings are low compared with expenses.",
            }
        )
        ...
        alerts.append(
            {
                "type": "OVER_BUDGET",
                "severity": "HIGH",
                "message": f"{over_budget_count} active budget item(s) are over budget.",
            }
        )

    if taken_remaining > 0 and net_worth <= 0:
        alerts.append(
            {
                "type": "DEBT_RISK",
                "severity": "HIGH",
                "message": "Outstanding payable loan exists while net worth is not positive.",
            }
        )
    elif taken_remaining > 0 and net_worth > 0:
        debt_ratio = (taken_remaining / net_worth) * Decimal("100")
        if debt_ratio >= Decimal("50"):
            alerts.append(
                {
                    "type": "HIGH_DEBT_RATIO",
                    "severity": "MEDIUM",
                    "message": "Payable loan is high compared with net worth.",
                }
            )

    if safe_decimal(g.get("active_goals", "0")) > 0 and goal_progress_percent < Decimal("25"):
        alerts.append(
            {
                "type": "LOW_GOAL_PROGRESS",
                "severity": "LOW",
                "message": "Goal progress is below 25%.",
            }
        )
        ...
        alerts.append(
            {
                "type": "MIXED_CURRENCY",
                "severity": "MEDIUM",
                "message": "Multiple wallet currencies detected. Executive totals should be reviewed with currency reports.",
            }
        )
    ...
    if total_expense == 0:
        if total_savings > 0:
            score += 20
    elif total_savings >= (total_expense * Decimal("0.25")):
        score += 20
    ...
    if goal_progress_percent >= Decimal("50"):
        score += 15
    elif goal_progress_percent >= Decimal("25"):
        score += 10

    if taken_remaining == 0:
        score += 10
    elif net_worth > 0:
        debt_ratio = (taken_remaining / net_worth) * Decimal("100")
        if debt_ratio <= Decimal("30"):
            score += 10
        elif debt_ratio <= Decimal("50"):
            score += 5
    ...
    return {
        "family_id": family_id,
        "metadata": {
            "report_name": "Executive Dashboard Report",
            "scope": "family_lifetime",
            "currencies_detected": currencies_detected,
        ...
        "savings_and_goals": {
            ...
            "goal_saved": money(goal_saved_amount),
            "goal_target": money(goal_target_amount),
            "goal_progress_percent": g.get("overall_progress_percent"),
        },
        "loans": {
            "given_remaining": money(given_remaining),
            "taken_remaining": money(taken_remaining),
            "total_remaining": money(loan_exposure),
            "receivable_remaining": money(given_remaining),
            "payable_remaining": money(taken_remaining),
            "net_loan_position": money(net_loan_position),
        },
        "budget_summary": {
            "active_budgets": active_budgets,
            "total_budget": money(total_budget_amount),
            "total_spent": money(total_budget_spent),
            "remaining": money(budget_remaining),
            "over_budget_count": over_budget_count,
            "usage_percent": safe_percent(budget_usage_percent),
        },
        "counts": {
            "members": members,
            "wallets": wallets,
            "active_goals": g.get("active_goals", 0),
            "active_loans": l.get("active_loans", 0),
            "active_budgets": active_budgets,
        },
        "top_income_category": category_summary.get("top_income_category"),
        "top_expense_category": category_summary.get("top_expense_category"),
...
def cash_flow_currency_report(
    ...
    base_currency = family.default_currency
    coa_cash_flow = accounting_service.generate_cash_flow(db, family_id)
    ...
    return {
        "family_id": family_id,
        "base_currency": base_currency,
        "engine": "accounting_service.generate_cash_flow",
        "coa_cash_flow": coa_cash_flow,
        "filters": {
            "start_date": start_date,
            "end_date": end_date,
        },
...
@router.get("/family-audit-currency/{family_id}")
def family_audit_currency(
    family_id: str,
    ...
    for item in rows:
        ...
        if item.member_id:
            member = (
                db.query(FamilyMember)
                .filter(
                    FamilyMember.id == item.member_id
                )
                .first()
            )

...
from app.services.permission_service import has_permission, require_permission
...
def get_any_active_member(db: Session, user_id: str) -> FamilyMember:
    member = (
    ...
    if not member:
        raise HTTPException(
            status_code=403,
            detail="You are not an active family member",
        )

    return member


def require_any_permission(db: Session, user_id: str, permission: str) -> FamilyMember:
    members = (
        db.query(FamilyMember)
        .filter(
            FamilyMember.user_id == user_id,
...
def list_currencies(
    ...
    get_any_active_member(db=db, user_id=current_user.id)
